document.addEventListener('DOMContentLoaded', function () {
  ...
  dropZone.addEventListener('drop', (event) => {
    ...
    if (files.length > 0) {
      ...
      itemFoundFileName.innerText = files[0].name;
    }
  });
  ...
  fileInput.addEventListener('change', (event) => {
    ...
    if (file) {
      ...
      dropZone.classList.add('hidden');
      itemFound.classList.remove('hidden');
      itemFoundFileName.innerText = file.name;
    }
  });
  ...
  function generateTextFile(data) {
    ...
    results.forEach((item) => {
      ...
      if (sender && transcript) {
        if (sender === 'C') sender = customerName;
        if (sender === 'A') sender = agentName;
        if (sender === 'B') sender = 'Bot';
